...
gronda_con=0
do while (gronda_con.eq.0)

!da indentare correttamente
...
pt_new=quota_min/(cnt_b_cell*delta_x*sqrt(2.0))

if (cqm_new.gt.0) then
   if (max_q_depit.ge.quota_min) then
      ...
      write(6,*) 'is recommended less then', cqm_new
      write(6,*)
      write(6,*) '********************************************************************************'
      stop
   end if
else
   if((cnt_b_cell*delta_x*sqrt(2.0)*pt).gt.quota_min) then
      write(6,*) '********************************************************************************'
      write(6,*) 'WARNING:'
      write(6,*)
      write(6,*) 'Probably after the depitting, the highest boundary channel cell has elevation'
      write(6,*) 'great then the lowest dem cell.'
      write(6,*) 'A new value of Depit threshold slope less then', pt_new,','
	  write(6,*) 'or a DEM upward shifting greater then', max_q_depit, 'is recommended.'
      write(6,*)
	  write(6,*) '********************************************************************************'
	  write(6,*)
	  write(6,*) 'Select:'
	  write(6,*) 
	  write(6,'(1x,a)') '0) program stops for hap.in correction'
      write(6,'(1x,a)') '1) program continues with automatic DEM shifting'      
      write(6,'(1x,a)') '(Ctrl C to exit)'
      write(6,*)
      write(6,'(1x,a3,$)') '-> '
      read(5,*,iostat=ic) psc
      write(6,*) 
      if (psc.eq.0) stop
      if (psc.eq.1) then
         write(6,*)
         write(6,'(1x,a)') 'Select shifting value:'
         write(6,'(1x,a)') '(Ctrl C to exit)'
         write(6,*)
         write(6,'(1x,a3,$)') '-> '
         read(5,*,iostat=ic) dem_shift	     
         write(6,*) '********************************************************************************'
         write(6,*) 'WARNING:'
         write(6,*)
         write(6,*) 'The DEM is shifted upward by', dem_shift, ' m.'
         write(6,*)
         write(6,*) '********************************************************************************'
		 gronda_con=0
		 do j=M,1,-1
            do i=1,N
               ...
               quota_cij=dtm_quota(i_basin)
               if (quota_cij < 0.0_REP) cycle
			   quota_cij=quota_cij+dem_shift
			   call set_quota(i_basin,quota_cij)
			end do
         end do
		 quota_min=quota_min+dem_shift
	  end if      
   end if
end if

!fine da indentare correttamente
