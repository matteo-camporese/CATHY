...
public :: dtm_upl           ! 41, Upward path length
public :: dtm_dpl           ! 42, Downward path length
...
integer(kind=ISP),parameter :: nbb_rsp=29
...
type cella
   ...
   real(kind=RSP) ::    upl           ! 41
   real(kind=RSP) ::    dpl           ! 42
end type
...
subroutine set_upl(i,x)
integer(kind=ISP),intent(in) :: i
real(kind=RSP),intent(in) :: x
dtm(dtm_index_ps(i))%upl=x
end subroutine

subroutine set_dpl(i,x)
integer(kind=ISP),intent(in) :: i
real(kind=RSP),intent(in) :: x
dtm(dtm_index_ps(i))%dpl=x
end subroutine
