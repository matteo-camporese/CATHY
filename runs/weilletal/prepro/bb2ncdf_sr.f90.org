...
call rbb_ncdf('dem',13,2,0.0_RSP,0)
...
subroutine rbb_ncdf(dtmfile,ibb,ht,nodata,ips)
...
integer(kind=ISP) ,dimension(1) :: var_id
...
!    
   status = nf90_create("test.nc", NF90_CLOBBER, file_id)
   ...
   status = nf90_put_var(file_id, var_id(1), rdtm)
