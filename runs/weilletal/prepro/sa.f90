...
type feat_stream
   real (kind=4) :: length         ! length of the link
   integer (kind=4) :: n_elem      ! number of nodes defining a link
   integer (kind=4) :: sso         ! Stralher stream order
   real (kind=4) :: area           ! upslope area
...
!-----------------------------------------------------------------------------
!
! SA (Strahler Analysis)
!
! This program perform a Strahler analysis of the drainage net based on
...
open(24,file='tab_sa.dat',status='unknown')
...
write(6,*) 'writing file strahler_net.shp'

call IniziaScrittura('strahler_net',3) ! 3 Polyline

db_main%NomeFile='strahler_net.dbf'
db_main%uni=40
db_main%nFields=4
...
do i=1,cnt_s_tot
   ...
   call Set_Field_integer(db_main, db_main%F(3)%FieldName, stream(i)%sso)
...
integer(kind=4) :: cm,cnt_n,cnt_s,ccsso
...
do while (cm.eq.1)
   cm=0 
   ! check correct strahler stream order: it must be 0 or 1
   ! if it is equal to 2 then there are at least two channel converging to 
   ! the node with the same sso of the resulting dowstream channel
   ccsso=0     
   do ii=i-1,i+1
      do jj=j-1,j+1
         ...
         if (p_inflow+p_outflow_1_ciijj.eq.10) then             
            if (dtm_sso(ii_basin).ne.dtm_sso(i_basin)) then
               call rcs(i,j,ii,jj)
            else
               if (stream(cnt_s)%n_elem > 0) &
               ...
               cnt_n=cnt_n+1 
               cm=1
               ! check of Strahler orders
               ccsso=ccsso+1
               if (ccsso >= 2) then
                  write(*,*) 'Wrong Strahler Stream Order in cell',i,j
               end if 
               i_cm=ii
               j_cm=jj
            end if
...
stream(cnt_s)%sso=dtm_sso(i_basin)
