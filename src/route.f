...
C
C***********************************************************************
      SUBROUTINE ROUTE(NROW,NCELL,TIPO_R,RESERVR,
     1     CELLS_R,N_HA,TIME,DELTAT)
      ...
      REAL*8 Q_IN_KK,Q_IN_KKP1,Q_OUT_KK,Q_OUT_KKP1
...
C LOCAL CONTRIBUTION TO SURFACE RUNOFF FROM FLOW3D,SURFACE_WATER

         SURFACE_WATER=SURFACE_WATER_SN(I_BASIN)/NRC
      
          
         
         IF (W_1.NE.0) THEN
   
C CALCULATION OF LATERAL INFLOW RATE, Q_OVERLAND

            CR=1.0D0/EPL_1
            Q_OVERLAND_1=SURFACE_WATER*W_1*CR
...
C     INFLOW AND OUTFLOW HYDROGRAPHS FOR MC.F OR SERBATIO.F      
            ...
            Q_IN_KK=Q_IN_KK_SN(I_BASIN)*W_1/NRC
            Q_OUT_KK=Q_OUT_KK_SN_1(I_BASIN)/NRC
            Q_IN_KKP1=Q_IN_KKP1_SN(I_BASIN)*W_1/NRC
...
!     CXCX          WRITE(IOUT41,*)'ATTENZIONE! PORTATE USCENTI NEGATIVE'
                  ...
                  Q_OUT_KKP1_SN_1(I_BASIN)=Q_OUT_KKP1*NRC
                  ...
                  IF (N_O_QUOTA.EQ.NCELL) WRITE(IOUT40,1500)TIME,I,J,CU,
     +                                                      PE,X,C1
           ...
           ELSE                  ! IF (TIPO_R(INDEX_C).GE.1000) THEN
              ...
              STOP
C             Q_OUT_KKP1=Q_IN_KKP1 + SURFACE_WATER             
C             Q_OUT_KKP1_SN_1(I_BASIN)=Q_OUT_KKP1
            END IF
            
C     SUPERIMPOSITION OF OUTFLOW HYDROGRAPHS FOR THE DOWNSTREAM CELLS
           IF (N_O_QUOTA .NE. NCELL) THEN
               Q_IN_KKP1_SN(I_BASIN_CV_1) = Q_IN_KKP1_SN(I_BASIN_CV_1) + 
     1              Q_OUT_KKP1_SN_1(I_BASIN)
            END IF                 
         END IF                 !  IF (W_1.NE.0)
...
C CALCULATION OF LATERAL INFLOW RATE, Q_OVERLAND
            CR=1.0D0/EPL_2
            Q_OVERLAND_2=SURFACE_WATER*W_2*CR
C CALCULATION OF DOWNSTREAM CELL COORDINATES            
...
C     INFLOW AND OUTFLOW HYDROGRAPHS FOR MC.F OR SERBATIO.F      
            Q_IN_KK=Q_IN_KK_SN(I_BASIN)*W_2/NRC
            Q_OUT_KK=Q_OUT_KK_SN_2(I_BASIN)/NRC
            Q_IN_KKP1=Q_IN_KKP1_SN(I_BASIN)*W_2/NRC     
...
c      WRITE(IOUT41,*)'ATTENZIONE! PORTATE USCENTI NEGATIVE'
                 ...
                 H_POOL_KKP1_VEC(RESERVR(I_BASIN))=H_POOL_KKP1
                 Q_OUT_KKP1_SN_2(I_BASIN)=Q_OUT_KKP1
...
C        MUSKINGUM-CUNGE
     ...
     1               Y1_SF,DELTAT,CU,PE,X,C1,AK,Q_IN_KK,Q_IN_KKP1,
     2                Q_OUT_KK,Q_OVERLAND_2,Q_OUT_KKP1)
...
!     CXCX          WRITE(IOUT41,*)'ATTENZIONE! PORTATE USCENTI NEGATIVE'
                  ...
                  Q_OUT_KKP1_SN_2(I_BASIN)=Q_OUT_KKP1*NRC
     ...
     +                                                      PE,X,C1
            ...
            ELSE                ! IF (TIPO_R(I_BASIN).GE.1000) THEN
               ...
               STOP              
C               Q_OUT_KKP1=Q_IN_KKP1 + SURFACE_WATER             
C               Q_OUT_KKP1_SN_2(I_BASIN)=Q_OUT_KKP1
            END IF
            

C     SUPERIMPOSITION OF OUTFLOW HYDROGRAPHS FOR THE DOWNSTREAM CELLS
            Q_IN_KKP1_SN(I_BASIN_CV_2)= Q_IN_KKP1_SN(I_BASIN_CV_2) + 
     1           Q_OUT_KKP1_SN_2(I_BASIN)
