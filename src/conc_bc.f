...
C
     ...
     2     CONCNODBC,CNNEW,CONTP_TRA,PRESC_TRA,ANP_TRA,CONCNODUPD)
C
      IMPLICIT NONE
      INTEGER  NNOD,I
...
C
               ...
               GO TO 500
            ELSEIF ((ATMPOT(I) .GE. 0.).AND. (ATMACT(I) .LT. 0.)) THEN
               VOLTOT(I) = PONDNOD(I) * ARENOD(I) + ATMPOT(I) * DELTAT -
     1              ATMACT(I) * DELTAT
c
               MASSURF(I) = PONDNOD(I) * ARENOD(I) * CONCNOD(I)
...
c            IF (IFATMP(I).EQ.0) THEN
            
C     RAINFALL/INFILTRATION CASE         
            IF ((ATMPOT(I) .GT. 0.) .AND. (ATMACT(I) .GE. 0.)) THEN
...
c     RAINFALL/INFILTRATION CASE 
c            ELSEIF ((IFATM(I) .NE. 2).AND. (IFATMP(I).NE.0) ) THEN
